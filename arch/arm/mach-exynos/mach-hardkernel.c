/* linux/arch/arm/mach-exynos4/mach-hardkernel.c
 *
 * Copyright (c) 2012 AgreeYa Mobility
...
#define HARDKERNEL_UCON_DEFAULT	(S3C2410_UCON_TXILEVEL |	\
...
#define HARDKERNEL_ULCON_DEFAULT	S3C2410_LCON_CS8

#define HARDKERNEL_UFCON_DEFAULT	(S3C2410_UFCON_FIFOMODE |	\
				 S5PV210_UFCON_TXTRIG4 |	\
				 S5PV210_UFCON_RXTRIG4)

static struct s3c2410_uartcfg hardkernel_uartcfgs[] __initdata = {
	[0] = {
		.hwport		= 0,
		.flags		= 0,
		.ucon		= HARDKERNEL_UCON_DEFAULT,
		.ulcon		= HARDKERNEL_ULCON_DEFAULT,
		.ufcon		= HARDKERNEL_UFCON_DEFAULT,
	},
	[1] = {
		.hwport		= 1,
		.flags		= 0,
...
static struct max8997_platform_data __initdata hardkernel_max8997_pdata = {
	.num_regulators = ARRAY_SIZE(hardkernel_max8997_regulators),
	.regulators	= hardkernel_max8997_regulators,
...
static struct s3c_sdhci_platdata hardkernel_hsmmc2_pdata __initdata = {
	.cd_type		= S3C_SDHCI_CD_INTERNAL,
};
 ...
 */
static int hardkernel_wifi_status_register(void (*notify_func)
		(struct platform_device *, int state));

/* WLAN: MMC3-SDIO */
static struct s3c_sdhci_platdata hardkernel_hsmmc3_pdata __initdata = {
	.max_width		= 4,
	.host_caps		= MMC_CAP_4_BIT_DATA |
			MMC_CAP_MMC_HIGHSPEED | MMC_CAP_SD_HIGHSPEED,
	.cd_type		= S3C_SDHCI_CD_EXTERNAL,
	.ext_cd_init		= hardkernel_wifi_status_register,
};
...
#define HARDKERNEL_WLAN_WOW EXYNOS4_GPX2(3)
#define HARDKERNEL_WLAN_RESET EXYNOS4_GPX2(4)


static void hardkernel_wlan_setup_power(bool val)
{
	int err;

	if (val) {
		err = gpio_request_one(HARDKERNEL_WLAN_RESET,
				GPIOF_OUT_INIT_LOW, "GPX2_4");
		if (err) {
			pr_warning("HARDKERNEL: Not obtain WIFI gpios\n");
			return;
		}
		s3c_gpio_cfgpin(HARDKERNEL_WLAN_RESET, S3C_GPIO_OUTPUT);
		s3c_gpio_setpull(HARDKERNEL_WLAN_RESET,
						S3C_GPIO_PULL_NONE);
		/* VDD33,I/O Supply must be done */
		gpio_set_value(HARDKERNEL_WLAN_RESET, 0);
		udelay(30);	/*Tb */
		gpio_direction_output(HARDKERNEL_WLAN_RESET, 1);
	} else {
		gpio_direction_output(HARDKERNEL_WLAN_RESET, 0);
		gpio_free(HARDKERNEL_WLAN_RESET);
	}
 ...
 */
static int hardkernel_wifi_set_detect(bool val)
{
	if (!wifi_status_cb) {
	...
	if (true == val) {
		hardkernel_wlan_setup_power(true);
		wifi_status_cb(&s3c_device_hsmmc3, 1);
	} else {
		hardkernel_wlan_setup_power(false);
...
struct ath6kl_platform_data hardkernel_wlan_data  __initdata = {
	.setup_power = hardkernel_wifi_set_detect,
};


/* USB EHCI */
static struct s5p_ehci_platdata hardkernel_ehci_pdata;

static void __init hardkernel_ehci_init(void)
{
	struct s5p_ehci_platdata *pdata = &hardkernel_ehci_pdata;

	s5p_ehci_set_platdata(pdata);
}

/* USB OHCI */
static struct exynos4_ohci_platdata hardkernel_ohci_pdata;

static void __init hardkernel_ohci_init(void)
{
	struct exynos4_ohci_platdata *pdata = &hardkernel_ohci_pdata;
...
static struct platform_device hardkernel_lcd_hv070wsa = {
	.name			= "lcd-pwrctrl",
	.dev.parent		= &s5p_device_fimd0.dev,
	.dev.platform_data	= &hardkernel_lcd_hv070wsa_data,
};

static struct s3c_fb_pd_win hardkernel_fb_win0 = {
	.xres			= 1024,
	.yres			= 600,
	.max_bpp		= 32,
	.default_bpp		= 24,
...
static struct fb_videomode hardkernel_lcd_timing = {
	.left_margin	= 64,
	.right_margin	= 16,
	.upper_margin	= 64,
	.lower_margin	= 16,
...
static struct s3c_fb_platdata hardkernel_lcd_pdata __initdata = {
	.win[0]		= &hardkernel_fb_win0,
	.vtiming	= &hardkernel_lcd_timing,
...
struct rfkill_gpio_platform_data hardkernel_bt_pdata = {
...
static struct platform_device hardkernel_device_bluetooth = {
	.name		= "rfkill_gpio",
	.id		= -1,
	.dev		= {
		.platform_data	= &hardkernel_bt_pdata,
	},
};
...
/* LCD Backlight data */
static struct samsung_bl_gpio_info hardkernel_bl_gpio_info = {
	.no		= EXYNOS4_GPD0(0),
	.func		= S3C_GPIO_SFN(2),
};

static struct platform_pwm_backlight_data hardkernel_bl_data = {
	.pwm_id		= 0,
	.pwm_period_ns	= 1000,
};

static void __init hardkernel_bt_setup(void)
{
...
static void __init hardkernel_map_io(void)
{
	exynos_init_io(NULL, 0);
	s3c24xx_init_clocks(24000000);
	s3c24xx_init_uarts(hardkernel_uartcfgs, ARRAY_SIZE(hardkernel_uartcfgs));
}

static void __init hardkernel_power_init(void)
{
...
static void __init hardkernel_machine_init(void)
{
	hardkernel_power_init();
	 ...
	 */
	s3c_sdhci2_set_platdata(&hardkernel_hsmmc2_pdata);
	s3c_sdhci0_set_platdata(&hardkernel_hsmmc0_pdata);
	s3c_sdhci3_set_platdata(&hardkernel_hsmmc3_pdata);

	hardkernel_ehci_init();
	hardkernel_ohci_init();
	s3c_hsotg_set_platdata(&hardkernel_hsotg_pdata);
	...
	s5p_fimd0_set_platdata(&hardkernel_lcd_pdata);

	platform_add_devices(hardkernel_devices, ARRAY_SIZE(hardkernel_devices));

	samsung_bl_set(&hardkernel_bl_gpio_info, &hardkernel_bl_data);

	hardkernel_bt_setup();

	ath6kl_set_platform_data(&hardkernel_wlan_data);
}
...
MACHINE_START(HARDKERNEL, "ODROIDPC")
	...
	.map_io		= hardkernel_map_io,
	.handle_irq	= gic_handle_irq,
	.init_machine	= hardkernel_machine_init,
	.timer		= &exynos4_timer,
	.reserve	= &hardkernel_reserve,
